...
PawnMovePolicy::PawnMovePolicy(PlayerKind piece_color)
    : MovePolicy(PieceKind::Pawn), m_pawn_color(piece_color) {
...
void PawnMovePolicy::allMoves(const Piece &piece, Chessboard &board,
  ...
  if (isPositionInChessboardBounds(crt_pos) && board.isTileEmpty(crt_pos)) {
    ...
    if (isInInitialRow(piece) && isPositionInChessboardBounds(crt_pos) &&
        board.isTileEmpty(crt_pos)) {
      result.push_back(Move{crt_pos, Move::Kind::Normal});
    }
  }
